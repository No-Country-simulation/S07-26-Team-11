 ...
 * una conexion real al pool y ejecuta una consulta. Es la forma rapida de distinguir
 * "la API esta caida" de "la API esta arriba pero no llega a la base".
 *
 * 200 -> conexion establecida. 503 -> no se pudo conectar.
 *
...
public class DatabaseStatusController {
    ...
    public DatabaseStatusController(DataSource dataSource,
                                    @Value("${app.db-status.show-details:false}") boolean showDetails) {
    ...
    public ResponseEntity<Map<String, Object>> databaseStatus() {
        ...
        try (Connection connection = dataSource.getConnection();
            ...
            // El detalle completo va al log del servidor; al cliente solo el codigo
            // estandar SQLState (08001 = no se pudo abrir la conexion, 28P01 = password
            // invalido, 3D000 = la base no existe). Nunca la traza ni el mensaje interno.
